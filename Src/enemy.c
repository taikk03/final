...
bool updateEnemy(Enemy * enemy, Map * map, Player * player){
    ...
    if(enemy->status == DYING){
        ...
        */ 
        ...
        offsetX = 16 * (int)(enemy->death_animation_tick / (64 / 8));
        offsetY = 16;
        if (offsetX / 16 >= limiter) offsetX = 16 * (limiter - 1);
        int dy = enemy->coord.y; // destiny y axis
        int dx = enemy->coord.x; // destiny x axis
        al_draw_tinted_scaled_bitmap(player->image, al_map_rgb(255, 255, 255),
            offsetX, offsetY, 32, 32, // source image x, y, width, height
...
void drawEnemy(Enemy * enemy, Point cam){
    ...
    if(enemy->status == ALIVE){
        ...
        */
        int offset = 16 * (int)(enemy->death_animation_tick / (64 / 8));
...
static bool isCollision(Point enemyCoord, Map* map){
    ...
    /* 
        ...
        if(!isWalkable(map->map[...][...])) return true;
    */
    int left = enemyCoord.x / TILE_SIZE;
    int top = enemyCoord.y / TILE_SIZE;
    int right = (enemyCoord.x + TILE_SIZE - 1) / TILE_SIZE;
    int bottom = (enemyCoord.y + TILE_SIZE - 1) / TILE_SIZE;
